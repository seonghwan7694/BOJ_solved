...
long long F[MAX_SIZE], A[41][MAX_SIZE]; // {k, i}
...
int main(){
    ...
    int n, k; cin >> n >> k;
    F[1] = 1, F[2] = 2;
    for(int i = 3; i < MAX_SIZE; i++) F[i] = F[i - 1] + F[i - 2];
    for(int k = 1; k < 41; k++){
    ...
    for(int i = 1, tmp = 0; i < MAX_SIZE; i++){
        v[i] = v[i - 1] + A[k][i];
        v[i] %= mod;
    }
    // for(int k = 1; k < 20; k++){ for(int i = 1; i < 20; i++){ cout << A[k][i] << " "; } cout << "\n"; }
